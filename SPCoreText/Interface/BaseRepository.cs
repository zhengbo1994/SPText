...
namespace SPCoreText.Interface
{
    ...
    {
        private HkoMSContext _context;

        public BaseRepository(HkoMSContext context)
        {
            _context = context;
        }
        ...
        /// 查找单个，且不被上下文所跟踪
        /// </summary>
        public T FindSingleLastOrDefault(Expression<Func<T, bool>> exp)
        {
            return _context.Set<T>().AsNoTracking().LastOrDefault(exp);
        }
        ...
        public DataSet ExecuteSqlReturnDataSet(string sql, params SqlParameter[] para)
        {
