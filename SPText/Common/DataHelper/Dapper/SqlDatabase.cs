    ...
    public class SqlDatabase : IDatabase
    {
        ...
        public int ExecuteBySql(string strSql)
        {
            if (dbTransaction == null)
            {
                ...
                {
                    return connection.Execute(strSql, null, null, CommandTimeout);
                }
            }
            else
            {
                dbTransaction.Connection.Execute(strSql, null, dbTransaction, CommandTimeout);
                return 0;
        ...
        public int ExecuteBySql(string strSql, params DbParameter[] dbParameter)
        {
            if (dbTransaction == null)
            {
                using (var connection = Connection)
                {
                    return connection.Execute(strSql, dbParameter, null, CommandTimeout);
                }
            }
            else
            {
                dbTransaction.Connection.Execute(strSql, dbParameter, dbTransaction, CommandTimeout);
                return 0;
        ...
        public int ExecuteByProc(string procName)
        {
            if (dbTransaction == null)
            {
                using (var connection = Connection)
                {
                    return connection.Execute(procName, null, null, CommandTimeout);
                }
            }
            else
            {
                dbTransaction.Connection.Execute(procName, null, dbTransaction, CommandTimeout);
                return 0;
        ...
        public int ExecuteByProc(string procName, params DbParameter[] dbParameter)
        {
            if (dbTransaction == null)
            {
                using (var connection = Connection)
                {
                    return connection.Execute(procName, dbParameter, null, CommandTimeout);
                }
            }
            else
            {
                dbTransaction.Connection.Execute(procName, dbParameter, dbTransaction, CommandTimeout);
                return 0;
        ...
        public int Delete<T>(object propertyValue, string propertyName) where T : class
        {
            ...
            IEnumerable<T> entitys = dbTransaction.Connection.Query<T>(string.Format("select * from {0} where {1}=@propertyValue", propertyName), new { propertyValue = propertyValue }, null, true, CommandTimeout);
        ...
        public T FindEntity<T>(object keyValue) where T : class
        {
            ...
            {
                var data = dbConnection.Query<T>("select * from {T表名} where key=@key", new { key = keyValue }, null, true, CommandTimeout);
        ...
        public T FindEntity<T>(Expression<Func<T, bool>> condition) where T : class, new()
        {
            using (var dbConnection = Connection)
            {
                var data = dbConnection.Query<T>(new SQLinq<T>().Where(condition), null, true, CommandTimeout);
                return data.FirstOrDefault();
            }
        }
        /// <summary>
        ...
        public IQueryable<T> IQueryable<T>() where T : class, new()
        {
            using (var dbConnection = Connection)
            {
                return (IQueryable<T>)dbConnection.Query<T>(new SQLinq<T>(), null, true, CommandTimeout);
            }
        }
        /// <summary>
        /// 未实现
        ...
        public IQueryable<T> IQueryable<T>(Expression<Func<T, bool>> condition) where T : class, new()
        {
            using (var dbConnection = Connection)
            {
                return (IQueryable<T>)dbConnection.Query<T>(new SQLinq<T>().Where(condition), null, true, CommandTimeout);
            }
        }
        public IEnumerable<T> FindList<T>() where T : class, new()
        {
            using (var dbConnection = Connection)
            {
                return dbConnection.Query<T>(new SQLinq<T>(), null, true, CommandTimeout).ToList();
            }
        }
        public IEnumerable<T> FindList<T>(Func<T, object> keySelector) where T : class, new()
        {
            using (var dbConnection = Connection)
            {
                return dbConnection.Query<T>(new SQLinq<T>(), null, true, CommandTimeout).OrderBy(keySelector).ToList();
            }
        }
        public IEnumerable<T> FindList<T>(Expression<Func<T, bool>> condition) where T : class, new()
        {
            using (var dbConnection = Connection)
            {
                return dbConnection.Query<T>(new SQLinq<T>().Where(condition), null, true, CommandTimeout).ToList();
            }
        }
        ...
        public IEnumerable<T> FindList<T>(string strSql, DbParameter[] dbParameter) where T : class
        {
            using (var dbConnection = Connection)
            {
                return dbConnection.Query<T>(strSql, dbParameter, null, true, CommandTimeout);
            }
        }
        public IEnumerable<T> FindList<T>(string orderField, bool isAsc, int pageSize, int pageIndex, out int total) where T : class, new()
        {
            using (var dbConnection = Connection)
            {
                ...
                var dataQuery = dbConnection.Query<T>(dataLinq, null, true, CommandTimeout);
        ...
        public IEnumerable<T> FindList<T>(string strSql, DbParameter[] dbParameter, string orderField, bool isAsc, int pageSize, int pageIndex, out int total) where T : class
        {
            using (var dbConnection = Connection)
            {
                ...
                total = Convert.ToInt32(new DbHelper(dbConnection) { CommandTimeout = CommandTimeout }.ExecuteScalar(CommandType.Text, "Select Count(1) From (" + strSql + ") As t", dbParameter));
                var IDataReader = new DbHelper(dbConnection) { CommandTimeout = CommandTimeout }.ExecuteReader(CommandType.Text, sb.ToString(), dbParameter);
        ...
        public DataTable FindTable(string strSql, DbParameter[] dbParameter)
        {
            using (var dbConnection = Connection)
            {
                var IDataReader = new DbHelper(dbConnection) { CommandTimeout = CommandTimeout }.ExecuteReader(CommandType.Text, strSql, dbParameter);
        ...
        public object FindObject(string strSql, DbParameter[] dbParameter)
        {
            using (var dbConnection = Connection)
            {
                return new DbHelper(dbConnection) { CommandTimeout = CommandTimeout }.ExecuteScalar(CommandType.Text, strSql, dbParameter);
            }
        }
        #endregion
    }
