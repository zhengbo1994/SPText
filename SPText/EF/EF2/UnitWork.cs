...
namespace SPText.EF.EF2
{
    ...
    {
        private ModelDbset _context;

        public UnitWork(ModelDbset context)
        {
            _context = context;
        }
        public ModelDbset GetDbContext()
        {
            return _context;
        }
        ...
        public IQueryable<T> Find<T>(Comparison<T> orderby, int pageindex, int pagesize, Expression<Func<T, bool>> exp = null) where T : class
        {
        ...
        {
            //_context.Set<T>().Attach(T);//将数据用未更改的状态放在上下文中，相当于从数据库读取了该实体
            //var entityMOdel = _context.Set<T>().Where(where);
